...
export function SobrePositioning() {
  const { positioning } = SOBRE_PAGE;
  ...
  return (
    <section className="relative bg-charcoal text-concrete py-20 md:py-28 lg:py-36 overflow-hidden">
      <Container width="default">
        ...
        >
          <DisplayHeadline bronze={positioning.bronze} bronzeItalic={positioning.bronzeItalic} align="left" />
        </motion.div>

        <div className="space-y-6 md:space-y-7 max-w-3xl">
          {positioning.paragraphs.map((p, i) => (
            <Reveal key={i} as="p" delay={0.3 + i * 0.1} className="font-body text-[clamp(17px,1.8vw,21px)] leading-[1.7] text-concrete/90">
              {p}
            </Reveal>
          ))}
        </div>
