...
export function EspecialistasPageHero() {
  const { hero, breadcrumb } = ESPECIALISTAS_PAGE;
      ...
      <Container width="wide" className="relative">
        ...
        >
          <ol className="flex items-center gap-2 font-body text-[12px] md:text-[13px] tracking-[0.05em] text-marmorino">
            ...
            <li aria-current="page" className="text-bronze">{breadcrumb.current}</li>
          </ol>
        </motion.nav>
        ...
        >
          ...
          <motion.p variants={FADE_UP} transition={TRANSITION_DEFAULT} className="mt-6 md:mt-8 font-display italic text-[clamp(20px,2.4vw,28px)] leading-snug text-marmorino max-w-2xl">
            {hero.description}
          </motion.p>
        </motion.div>
      </Container>
